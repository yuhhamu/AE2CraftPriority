...
- `Knowledge/ae2craftpriority-windows-build.md`
...
「AE2 Utility」「Not Enough Energistics」はMinecraft 1.20.1 Forge向けビルドが存在しない
(前者はNeoForge 1.21.1のみ、後者はForge 1.12.2止まりで開発終了)ため未対応。
...
Mega CellsはVanillaの `appeng.block.crafting.AbstractCraftingUnitBlock` を継承した独自の
`ICraftingUnitType`(`MEGACraftingUnitType`)を追加しているだけで、専用のCraftingCPUCluster/
CraftingServiceクラスは持たない。「MEGA Crafting CPU」は内部的にVanillaの `CraftingCPUCluster`/
`CraftingService` をそのまま使っているため、既存の `mixin/core/CraftingServiceMixin` が無改修で
効く。追加のMixinは不要(専用の `compat/` サブパッケージも存在しない)。

### ExtendedAE / Applied Mekanistics

どちらも標準AE2公開APIの範囲でPattern Provider等を追加しているだけで、CraftingService/
CraftingCPUClusterへのMixinは持たない。追加コード不要(専用の `compat/` サブパッケージも
存在しない)。
...
あるため、ソース確認だけで判断せず、疑わしい箇所は実機で検証すること。
...
日本語コメントを含むソースをWindows環境でビルドするため、`build.gradle` では
`tasks.withType(JavaCompile) { options.encoding = 'UTF-8' }` と
`processResources { filteringCharset = 'UTF-8' }` を明示している(前者はコンパイラ、
後者は `mods.toml` 等へのトークン置換用で、両方必要)。Mixin対象はほぼ全てAE2(非Vanilla)
クラスのため、対象クラスへの全Mixinに `remap = false` を付けている。
...
- CPU一覧の "@優先度" 表記は、選択中・未選択に関わらず全CPUに表示される
