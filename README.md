...
This branch is for **Minecraft 1.16.5**. See below for other versions.

このブランチは **Minecraft 1.16.5** 向けです。他のバージョンは以下を参照してください。
...
- Minecraft 1.16.5
- Minecraft Forge 36.1.10 or later
- Applied Energistics 2 for Minecraft 1.16.5 (8.4.x series; tested with 8.4.7)

AE2 does not distribute a build for 1.16.5 on Modrinth, so you'll need to get it from CurseForge instead.

This mod includes code that directly relies on AE2's internal implementation (not its public API). Any AE2 build for Minecraft 1.16.5 (8.4.x series) shares the same internal structure, so other 8.4.x versions are likely to work too — but only 8.4.7 has actually been verified to launch and run correctly. AE2 builds for other Minecraft versions (which use a different internal structure) are not supported.

- Minecraft 1.16.5
- Minecraft Forge 36.1.10以降
- Applied Energistics 2 の Minecraft 1.16.5向けバージョン(8.4.x系列。8.4.7で動作確認済み)

AE2は1.16.5向けビルドをModrinthに配信していないため、CurseForgeから取得してください。

このMODはAE2の内部実装(公開APIではない部分)を直接利用しているコードを含みます。Minecraft 1.16.5向けのAE2(8.4.x系列)であれば同じ内部構造を使っているため8.4.7以外でも動作する可能性が高いですが、実際に起動・動作確認できているのは8.4.7のみです。他のMinecraftバージョン向けのAE2(内部構造が異なります)には対応していません。
...
Right-click a Crafting CPU block to open its screen; a wrench-icon tab button has been added at the top right, immediately to the left of AE2's own "back" tab. Press it to open that CPU's priority screen (the back button returns you to the CPU screen).

Crafting CPUのブロックを右クリックしてCPU画面を開くと、画面右上、AE2純正の「戻る」タブのすぐ左にレンチアイコンのタブボタンが追加されています。これを押すとそのCPUの優先度画面が開きます(戻るボタンでCPU画面へ戻れます)。

### Method 3: Change it from the terminal's "Crafting Status" tab / 方法3: 端末の「クラフト状況(Crafting Status)」タブから変更する

In an AE2 terminal's Crafting Status tab, select the target CPU from the list on the left, then press the wrench-icon button added to the left of the "Cancel" button to open that CPU's priority screen. The back button's icon and label have been changed to a hammer icon (ported from AE2's own graphics) and "Crafting Status" so it correctly returns you to the Crafting Status tab.
...
AE2端末のクラフト状況タブで、左側のCPU一覧から対象CPUを選択し、「キャンセル」ボタンの左に追加されたレンチアイコンのボタンを押すと、選択中CPUの優先度画面が開きます。「戻る」ボタンのアイコン・ラベルはAE2純正のグラフィックから移植したハンマーアイコンと「Crafting Status」に差し替えてあり、正しくクラフト状況タブへ戻れるようになっています。
...
On this branch (1.16.5), compatibility with other AE2 addon mods (AdvancedAE, Mega Cells, etc.) has not been tested — only AE2 by itself has been verified on real hardware. (The 1.20.1 version has been verified alongside several companion mods; see the README on the `main` branch for details.)

このブランチ(1.16.5)では、AE2本体以外の連携MOD(AdvancedAE、Mega Cells等)を組み合わせた動作確認は行っていません。AE2単体との組み合わせでのみ実機確認済みです。(1.20.1版では複数の連携MODとの併用を確認済みです。詳細はmainブランチのREADMEを参照してください)
...
- AE2's own "Priority" screen (used by methods 2 and 3 above) has some hint text ("Insertion: Higher priority first" / "Extraction: Lower priority first") that AE2 itself has never translated into Japanese for this Minecraft version. This is a gap in AE2's own localization files, not something this mod controls.
...
- 上記の方法2・3で開くAE2純正の「優先度」画面には、一部のヒント文言(「Insertion: Higher priority first」「Extraction: Lower priority first」)があり、このMinecraftバージョン向けのAE2本体側で日本語訳が用意されていません。これはAE2本体のローカライズファイルの不備であり、本MODが制御できる範囲外です。
